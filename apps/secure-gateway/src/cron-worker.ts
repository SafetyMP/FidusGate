...
export function startConsensusExpiryWorker(
  ...
  console.error('⏰ Consensus Expiry background worker successfully booted.');
    ...
    try {
      ...
      for (const action of pending) {
        if (action.status === 'pending' && new Date(action.expiresAt).getTime() < now) {
          console.error(`⏰ CONSENSUS EXPIRED: Action ID: ${action.id} has passed its expiration time (${action.expiresAt}).`);
